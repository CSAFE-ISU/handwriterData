#' Cluster Template
#'
#' A cluster template created by the handwriter R package.
#'
#' The handwriter R package splits handwriting samples into component shapes
#' called *graphs*. The graphs are sorted into clusters with a K-Means
...
#'   for each graph used to create the cluster template. The clusters are numbers 1 through 40.}
#' \item{centers}{The final cluster centers produced by the K-Means algorithm.}
#' \item{K}{The number of clusters to build with the K-means algorithm.}
...
#' template_data <- handwriter::format_template_data(template)
